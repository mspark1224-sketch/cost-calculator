...
function saveRecipe() {
  ...
  const existing = products.find((p) => String(p.name) === String(name));
  ...
  if (existing) {
    ...
    products = products.map((p) => (String(p.name) === String(name) ? newData : p));
  } else {
...
function loadProducts() {
  ...
  filtered.forEach((p) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button onclick="loadProduct('${escapeJsString(String(p.id))}')">불러오기</button>
        <button class="danger" onclick="deleteProduct('${escapeJsString(String(p.id))}')">삭제</button>
      </td>
    `;
...
function loadProduct(id) {
  ...
  if (densityEl) densityEl.value = product.density ?? 1;
...
function loadMaterialCostFromProduct() {
  ...
  if (!selectedId) {
    ...
    if (materialCostText) materialCostText.innerText = "0";
    return;
  }
...
function calculate() {
  const selectedId = document.getElementById("calcProductSelect")?.value;
  const product = products.find((p) => String(p.id) === String(selectedId));
...
function loadQuotes() {
  ...
  filtered.forEach((q) => {
    ...
    tr.innerHTML = `
      ...
      <td><button class="danger" onclick="deleteQuote('${escapeJsString(String(q.id))}')">삭제</button></td>
    `;
